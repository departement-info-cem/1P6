...
Vous devez écrire un programme qui simule une calculatrice simple pour laquel on pourra spécifier la précision. Vous devrez créer une classe **`Calculatrice`** dans votre bibliothèque de classe **`Modele`** qui sera utilisée par votre interface graphique pour:
...
:::danger
Pour cet exercice, il est interdit de faire la convertion en décimale des deux chiffres dans l'interface graphique. Il faudra que la classe calculatrice reçoive en paramètre et retourne le résultat en format **`string`**.
:::
...
- Spécifier la précision désiré
- Entrer un premier chiffre dans un **`Textbox`**
- Entrer un deuxième chiffre dans un **`Textbox`**
- Quatre boutons pour chaque opération addition, soustraction, multiplication et divison
