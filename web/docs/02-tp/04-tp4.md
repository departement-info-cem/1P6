...
- Crée une fonction `RevelerTout` qui affichera toutes les cases de la grille lorsque l'on clic sur une mine :
...
- Nous avons déjà initialisé l'heure de début de la partie
```csharp
private void InitialiserGrille()
{
    //...
...
Fonction VerifierVictoire :
    ...
    Pour chaque ligne de 0 à lignesGrille - 1 :
        Pour chaque colonne de 0 à colonnesGrille - 1 :
            ...
            → Si la case (ligne, colonne) ne contient une mine
...
→ Si c'est une victoire :
    → Calculer la durée de la partie (TimeSpan duree = DateTime.Now - heureDebut;)
    → Afficher un MessageBox.Show($"🎉 Victoire ! Temps : {duree.TotalSeconds:F1} secondes");
...
## Étape 7 - Afficher les meilleurs temps
...
Initialiser tous les meilleurs temps à Anonyme;999
```text
Anonyme;999
Anonyme;999
Anonyme;999
...
### 2️⃣ Gérer le click `Meilleurs Temps` du menu

Dans le gestionnaire `meilleursTempsToolStripMenuItem_Click` :

```csharp
Fonction meilleursTempsToolStripMenuItem_Click(object sender, EventArgs e) :

    → Initialiser une chaîne vide « chemin »

    → Si le menu de difficulté Facile est coché :
          chemin = "../../../scores_facile.txt"
    
    → Sinon, si le menu de difficulté Moyenne est coché :
          chemin = "../../../scores_moyen.txt"
    
    → Sinon (on suppose que Difficile est coché) :
          chemin = "../../../scores_difficile.txt"

    → Appeler la
```
### 3️⃣ Afficher les meilleurs temps

Créer la fonction `AfficherMeilleursTemps`

```text
Fonction AfficherMeilleursTemps(cheminFichier) :

    → Si le fichier « cheminFichier » n’existe pas :
          afficher un MessageBox "Aucun résultat"
          arrêter la fonction

    → Initialiser un string vide « temps »
    → Ouvrir le fichier avec le chemin reçu en paramètre

    → Tant qu’on n’est pas à la fin du fichier :
          → Lire une ligne du fichier
          → Séparer la ligne en utilisant le caractère ';'
                parties = ligne.Split(';')

          → Ajouter au texte « temps » :
                "🏅 " + parties[0] + " ⏱️ " + parties[1] + " s" + retour à la ligne

    → Fermer le fichier

    → Afficher un MessageBox avec :
          - le texte « temps »
          - le titre "🏆 Meilleurs
```
...
- Cliquer sur **Meilleurs temps** affiche une boîte de dialogue listant les scores enregistrés pour le niveau en cours.

---

## Étape 8 - Sauvegarder les meilleurs temps
...
## Bonus - Ouverture en cascade des cases vides (version récursive)
