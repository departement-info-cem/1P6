...
Ce travail te fera passer de la console aux **interfaces graphiques (GUI)** tout en réinvestissant les notions de tableaux 2D, de boucles, de conditions et de lecture/écriture de fichiers.
...
bool[,] gGrilleRevelee;        // indique si une case est révélée
...
{
    MessageBox.Show("Nouvelle partie ! (logique à implémenter)");
}
...
private void facileToolStripMenuItem_Click(object sender, EventArgs e)
{
    ...
    MessageBox.Show("Difficulté : Facile");
}
...
private void moyenToolStripMenuItem_Click(object sender, EventArgs e)
{
    ...
    MessageBox.Show("Difficulté : Moyen");
}
...
private void difficileToolStripMenuItem_Click(object sender, EventArgs e)
{
    ...
    MessageBox.Show("Difficulté : Difficile");
}

private void afficherMeilleursTempsToolStripMenuItem_Click(object sender, EventArgs e)
{
    MessageBox.Show("Affichage des meilleurs temps à implémenter.");
}
```
...
- Changer de difficulté met à jour `gTailleGrille` et `gNombreMines` et affiche un message de confirmation.
...
### 1️⃣ Préparer une fonction `CreerGrilleBoutons()`
...
private void CreerGrilleBoutons()
{
    ...
    this.Controls.Clear();
    this.Controls.Add(menuPrincipal); // Réajouter le menu
    ...
    for (int ligne = 0; ligne < gTailleGrille; ligne++)
    {
        for (int colonne = 0; colonne < gTailleGrille; colonne++)
        {
            ...
            // Position de la case grâce à un Point(x, y)
...
Ajoute une méthode pour gérer le clic sur une case :
...
### 2️⃣ Appeler `CreerGrilleBoutons()`
...
private void nouvellePartieToolStripMenuItem_Click(object sender, EventArgs e)
{
    InitialiserGrille();   // méthode à créer à l’étape suivante
    CreerGrilleBoutons();
}
```

Pour l’instant, crée une version minimale de `InitialiserGrille()` :
...
- La taille de la fenêtre **change** correctement si tu passes de *Facile* à *Difficile* (8x8 → 16x16).  
...
- Placer aléatoirement les **mines** dans `gGrilleMines`.
...
- Marquer la case comme **révélée** dans `gGrilleRevelee`.
...
### 2️⃣ Utiliser `RevelerCase` dans `BoutonCase_Click`
